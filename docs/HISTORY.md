# Life Quest Journal — Release History & Archaeology

> **Archive.** This file holds the shipped-work narratives that used to
> live in `ROADMAP.md`: design rationale, per-day timelines, reverted
> experiments, audit logs, and as-built notes for Phase 1 (v2.4)
> through the in-flight v2.9 work. It was split out on **Jun 11, 2026**
> so the roadmap could stay a lean forward-looking plan.
>
> - Forward plan: `ROADMAP.md` (repo root)
> - Per-release notes: `CHANGELOG.md` (repo root)
> - Section numbers (§1.x, §2.x…) are preserved from the original
>   roadmap so existing cross-references in code comments, tests, and
>   the changelog still resolve.

---

## Feature Inventory (snapshot as of Jun 11, 2026 — live copy in ROADMAP.md)
...
### 1.7 — Streak Protection & Recovery (still planned — not shipped)

Forward-looking design; canonical (and maintained) version lives in
`ROADMAP.md` §1.7. Not duplicated here to prevent drift.
...
### 2.1 — View Transitions & Navigation Polish (v2.5) ✅ Shipped (v2.5.0, May 24, 2026)

See `CHANGELOG.md` § 2.5.0 for the shipped breakdown. (Note: the
active nav-link pulse and selected-tab glow shipped here were later
intentionally REMOVED by the v2.7 Motion Calm Pass — see § 2.3 —
in favor of static box-shadow glows.)
...
### 2.5 — Loot, Chest & Boss Battle Visuals + Theme of the Week (v2.9) 🚧 In Development

**Status (Jun 11, 2026):** Tracks 3, 4, 5, 6, 7 are implemented in the
codebase with regression tests (verify via the `v2.9 Track N` comment
anchors in `goal-manager.js` / `effects-manager.js` / `animations.css`).
Tracks 1 (chest sequences) and 2 (boss portraits) are NOT started —
both are asset-gated (see D1/D2 under Open dependencies). v2.9 has not
shipped to the Play Store yet; the released build remains 2.8.0.

- Custom chest-open sequences per tier (animated lid + light shafts + sequential reveal of items) — ⏳ pending (Track 1)
- Boss portraits / illustrated art (replacing emoji-only boss representation in the arena) — ⏳ pending (Track 2)
- Boss HP bar with animated damage chunks (red flash where damage hit, drained segments) — ✅ shipped as a damage-trail bar (Track 3; see track note)
- Crit / weakpoint visual indicators on attack — ✅ shipped (Track 4)
- Defeat sequence: boss dissolves into themed particles + loot fountain into the inventory — ✅ shipped (Track 5)
- Monthly boss final blow: brief slow-motion frame for the killing strike — ✅ shipped (Track 6)
- **"Theme of the Week" rotation card on the Dashboard** ✅ SHIPPED
  Jun 9, 2026 (v2.9 Track 7 — see detail block below for the full
  Q1–Q7 resolution and per-method shipped notes). 9-week deterministic
  rotation across the premium roster, in-week premium-bypass via
  `getWeeklyFeaturedThemeId()`, Monday spotlight modal, and end-of-
  trial revert+upsell all landed. Q6 (premium-purchase wiring) audit
  resolved in favor of the existing `showPremiumPurchaseModal()` —
  no v2.9.x deferral needed. Q7 analytics events have since been
  wired as well (see the Track 7 block below). Originally deferred
  from v2.8 (Jun 7, 2026 evening) and lived as the trailing
...
##### Track 1 — Per-tier chest-open sequences ⏳ NOT STARTED
...
##### Track 2 — Boss portraits / illustrated art ⏳ NOT STARTED
...
##### Track 3 — Boss HP bar with animated damage chunks ✅ SHIPPED (in code)

> **As-built note:** implementation diverged from the chunk-segmented
> plan below. Shipped as a **two-layer damage-trail bar**
> (`renderBossHPBar()` + rewritten `updateBossHPBar()` at
> `goal-manager.js` — search `v2.9 Track 3`): a `.boss-hp-damage`
> white-flash trail layer behind the live `.boss-hp-fill` gradient,
> draining to the new HP with a delay so each hit reads as a visible
> chunk of loss. Crit hits tint the flash yellow vs. normal red. The
> phase color/label mapping was factored into a shared helper so the
> bar cross-fades phases mid-fight without a full re-render.
...
##### Track 4 — Crit / weakpoint visual indicators ✅ SHIPPED (in code)

> **As-built:** landed per plan — weakpoint reticle, crit shockwave,
> and time-dilation layers in `effectsManager.bossCrit()`
> (`effects-manager.js` — search `v2.9 Track 4`; keyframes in
> `animations.css`).
...
##### Track 5 — Defeat sequence (themed particle dissolve + loot fountain) ✅ SHIPPED (in code)

> **As-built:** boss theme definitions carry a `particleType` field
> (5 palettes: shadow / ember / slime / frost / radiant) resolved via
> `getBossParticleType()` with a name-lookup fallback for older saves.
> `effectsManager.bossDefeatDissolve()` runs immediately on the still-
> mounted boss card; `effectsManager.lootFountain()` arcs loot icons
> into the player avatar ring with an onArrive callback that opens the
> loot panel. The full-screen celebration overlay is deferred so the
> dissolve has a clear window. Search `v2.9 Track 5` in
> `goal-manager.js` / `effects-manager.js`.
...
##### Track 6 — Monthly boss final blow (slow-motion frame) ✅ SHIPPED (in code)

> **As-built:** implemented as a **FINAL BLOW! overlay + vignette**
> that runs in parallel with the Track 5 dissolve for ~1.2s, monthly
> bosses only (search `v2.9 Track 6` in `goal-manager.js`). The
> planned `body.slow-motion` global animation-duration override was
> not used — the risk note below (breaking toast/modal animations)
> won out.
...
##### Track 7 — Theme of the Week rotation card ✅ SHIPPED Jun 9, 2026

**Final scope landed:**
- `getISOWeekNumber()` + `getWeeklyFeaturedThemeId()` — deterministic
  9-week rotation across the premium roster
  (`sunken → desert → stormwatch → volcanic → verdant → mystic →
  aurora → cathedral → crystal`), driven by `weeklyRotationIndex`
  fields on each premium theme. Same featured id for every user in
  the same ISO week, zero server roundtrip.
- `maybeShowFeaturedThemeSpotlight()` — Monday-morning trial-start
  modal, capped at one prompt per ISO week via
  `weeklyTrialPromptShown[weekKey]`. Apply button bypasses the
  premium gate for the duration of the featured week.
- `selectTheme()` + `renderThemes()` + `renderThemeSelector()` —
  featured-week premium bypass + `.theme-featured` badge/glow on
  the matching tile across both surfaces.
- `checkExpiredThemeTrial()` + `showThemeTrialEndedModal()` +
  `dismissThemeTrialEndedModal()` — Q3 end-of-trial loop. Fires on
  boot after `checkRewardUnlocks()`, one-shot per theme via
  `weeklyTrialEndPromptShown[themeId]`, two CTAs (Subscribe →
  `showPremiumPurchaseModal()`, Keep default → revert). Always
  reverts `currentTheme` to `default` on dismiss so a non-premium
  user can never linger on a premium-locked theme.
- Tests: 12 new cases under `Theme of the Week (v2.9 Track 7) →
  checkExpiredThemeTrial` in `tests/goal-manager.test.js` (all 5
  guard-condition negatives + 5 dismissal paths + idempotency +
  save/load round-trip). All passing.

**Q6 audit result:** the existing premium upsell entry is
`showPremiumPurchaseModal()` (used app-wide by spells,
enchantments, theme tiles). No `purchasePremium()` method exists,
no stub. Track 7 wires the trial-ended Subscribe CTA through it
directly — no v2.9.x deferral.

**Q7 analytics — ✅ SHIPPED post-Track-7 (closes the original "NOT
shipped" gap):** `weekly_theme_featured` (once per ISO week on first
launch, idempotent via `lastFeaturedWeekTracked`),
`weekly_theme_applied`, and `weekly_theme_subscribe` (within-14-days
funnel attribution) are wired in `goal-manager.js` — search
`v2.9 Track 7 Q7`. All firings guarded with `typeof trackEvent`
checks. The funnel hypothesis is measurable from day one of v2.9.

**NOT shipped in Track 7 (still TODO before v2.9 final):**
- **Q5 dashboard card surface** — the persistent between-summary-
  and-habits dashboard slot was descoped; the Monday spotlight
  modal + the in-grid `.theme-featured` badge are the only
  surfaces currently. Reconsider if user testing flags low
  awareness.

---

Original design-Q resolutions (preserved for posterity):
...
##### Suggested ordering *(historical — written Jun 8; actual
execution followed it for Tracks 3→4→5→6→7, with Track 7 landing
before the asset-gated Tracks 1–2 rather than after)*
...
##### v2.9 mid-cycle audits (Jun 10–11, 2026) ✅ Landed in code

Two audit passes ran against the in-development v2.9 build; all
fixes carry regression tests in `tests/goal-manager.test.js`
(`v2.9.x Audit Fixes` + `v2.9.x Android UX Fixes` describe blocks).
Fold these into the v2.9.0 release notes.

**Persistence / date-math audit (Jun 10)** — archive prune is a pure
500-cap (dead 6-month cutoff removed); `lastHabitReset` advanced only
by `checkHabitReset` (saves persist the stored value); week keys use
the ISO week-YEAR (`getISOWeekYear`) so New Year boundary weeks don't
collide; `flushPendingSave` writes pending debounced saves immediately
on `visibilitychange:hidden` (Android WebView kill-safety); corrupted-
save backups capped at one rotating key; `loadData` double-assignment
of `dailyQuestBoard` et al. removed.

**Android UX audit (Jun 11)** — full UI/UX pass with the Android-only
user base as the lens. Fixes landed same day:
- **Hardware back button handled** (was completely unhandled —
  pressing back over a modal minimized the app). New
  `handleHardwareBack()` / `closeTopOverlay()` in `goal-manager.js` +
  `CapBridge.registerBackButton()` in `capacitor-bridge.js`:
  close topmost overlay → navigate to dashboard → minimize.
  Back over a confirm dialog always cancels, never confirms.
- **Search consolidation** — removed the dead static `#search-modal`
  from `index.html` whose duplicate `search-modal`/`search-results`
  ids swallowed the dynamic search modal's results into a hidden
  container (search appeared broken). `openSearchModal()` /
  `closeSearch()` are now the single implementation; the document-
  level Escape listener no longer leaks on backdrop/✕ dismissal.
- **Destructive-confirm safety** — `showConfirm` now focuses Cancel
  instead of Confirm so a stray double-tap can't land on
  "Permanently delete?".
- **TalkBack nav labels** — `aria-label` on all 10 sidebar nav links
  (mobile CSS hides the text spans with `display:none`, which
  removed them from accessible-name computation — the bottom tab
  bar was ten unnamed buttons for TalkBack users).

**Audit findings deferred (tracked for v2.9/v2.10):**
- **Bundle Tailwind/icons/fonts locally** — `index.html` loads the
  Tailwind runtime compiler + Remix Icons + Google Fonts from CDNs;
  `scripts/copy-web.js` copies it verbatim, so a first launch with
  no network renders a completely unstyled app, and every cold
  start pays CDN latency + in-WebView JIT compilation. Needs a
  build-time Tailwind CSS step (standalone CLI keeps the
  no-bundler setup) + a dynamic-class safelist (`renderArchives`
  uses `from-${color}-900` template classes).
- **Toast "UNDO" action for deletes** — Ctrl+Z undo is unreachable
  on Android, making the undo system dead code for the entire user
  base while deletes remain confirm-only. Add a tappable UNDO
  button to the delete toast.
- **Toast tap-to-dismiss** (= L4 in the deferred-findings table).

---

*End of archive. Forward-looking work (v2.10 sound design, Phases 3-6, deferred UX findings, backlog) lives in `ROADMAP.md` at the repo root. Per-release notes live in `CHANGELOG.md`.*
